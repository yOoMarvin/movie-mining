...
check = [elem for elem in metadata.columns.values if elem.startswith("id")] + [elem for elem in metadata.columns.values
                                                                               if elem.startswith("index")]
print(status + "Check for suspicious index columns: {}".format(check))

print(status + "Preprocessing time: %.2f" % (time() - start))
